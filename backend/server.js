...
const blockSchema = new mongoose.Schema(
  {
    ...
    difficulty: { type: String, enum: ['easy', 'medium', 'hard', 'tough'] },
...
app.delete('/api/projects/:projectId', authMiddleware, async (req, res) => {
  try {
    ...
    if (!project) return res.status(404).json({ error: 'Project not found or not authorized' });
    await Block.deleteMany({ projectId: req.params.projectId });
    await Connection.deleteMany({ projectId: req.params.projectId });
    await Comment.deleteMany({ projectId: req.params.projectId });
    await ActivityLog.deleteMany({ projectId: req.params.projectId });
    await Version.deleteMany({ projectId: req.params.projectId });
    res.json({ message: 'Project deleted' });
  } catch (error) { res.status(500).json({ error: error.message }); }
});

// GET SINGLE BLOCK
app.get('/api/blocks/:blockId', authMiddleware, async (req, res) => {
  try {
...
app.post('/api/connections', authMiddleware, async (req, res) => {
  try {
    const { projectId, fromBlockId, toBlockId, connectionType, label } = req.body;
    const connection = new Connection({ projectId, fromBlockId, toBlockId, connectionType: connectionType || 'arrow', label });
    await connection.save();
    await Project.findByIdAndUpdate(projectId, { $push: { connections: connection._id }, activityAt: new Date() });
    await new ActivityLog({ projectId, userId: req.user._id, action: 'created_connection', changes: { fromBlockId, toBlockId } }).save();
...
app.delete('/api/connections/:connectionId', authMiddleware, async (req, res) => {
  try {
    ...
    await Project.findByIdAndUpdate(connection.projectId, { $pull: { connections: connection._id } });
    await new ActivityLog({ projectId: connection.projectId, userId: req.user._id, action: 'deleted_connection' }).save();
    res.json({ message: 'Connection deleted' });
  } catch (error) { res.status(500).json({ error: error.message }); }
});

// COMMENT ROUTES
app.get('/api/blocks/:blockId/comments', authMiddleware, async (req, res) => {
  try {
    const comments = await Comment.find({ blockId: req.params.blockId })
      .populate('authorId', 'name email avatar').sort({ createdAt: -1 });
    res.json(comments);
  } catch (error) { res.status(500).json({ error: error.message }); }
});

app.post('/api/blocks/:blockId/comments', authMiddleware, async (req, res) => {
  try {
    const { content, projectId, mentions } = req.body;
    const comment = new Comment({ blockId: req.params.blockId, projectId, authorId: req.user._id, content, mentions: mentions || [] });
    await comment.save();
    await comment.populate('authorId', 'name email avatar');
    await new ActivityLog({ projectId, userId: req.user._id, action: 'added_comment', blockId: req.params.blockId }).save();
...
app.delete('/api/comments/:commentId', authMiddleware, async (req, res) => {
  try {
    const comment = await Comment.findOneAndDelete({ _id: req.params.commentId, authorId: req.user._id });
...
app.get('/api/projects/:projectId/versions', authMiddleware, async (req, res) => {
  try {
    const versions = await Version.find({ projectId: req.params.projectId })
      .populate('creatorId', 'name email').sort({ createdAt: -1 }).limit(20);
    res.json(versions);
  } catch (error) { res.status(500).json({ error: error.message }); }
});
...
app.post('/api/projects/:projectId/versions', authMiddleware, async (req, res) => {
  try {
    ...
    const project = await Project.findById(req.params.projectId).populate('blocks').populate('connections');
    if (!project) return res.status(404).json({ error: 'Project not found' });
    const version = new Version({
      projectId: req.params.projectId, creatorId: req.user._id,
      message: message || 'Manual snapshot',
      snapshotData: { blocks: project.blocks, connections: project.connections }
    });
    await version.save();
...
app.post('/api/projects/:projectId/members', authMiddleware, async (req, res) => {
  try {
    ...
    const alreadyMember = project.teamMembers.find(m => m.userId.equals(memberUser._id));
    if (alreadyMember) return res.status(400).json({ error: 'Already a member' });
    project.teamMembers.push({ userId: memberUser._id, role: role || 'editor' });
    await project.save();
    await new ActivityLog({ projectId: req.params.projectId, userId: req.user._id, action: 'added_member', changes: { memberId: memberUser._id } }).save();
    res.json(project);
  } catch (error) { res.status(500).json({ error: error.message }); }
});
...
app.delete('/api/projects/:projectId/members/:memberId', authMiddleware, async (req, res) => {
  try {
    const project = await Project.findOne({ _id: req.params.projectId, ownerId: req.user._id });
    if (!project) return res.status(403).json({ error: 'Not authorized' });
    project.teamMembers = project.teamMembers.filter(m => !m.userId.equals(req.params.memberId));
    await project.save();
    await new ActivityLog({ projectId: req.params.projectId, userId: req.user._id, action: 'removed_member', changes: { memberId: req.params.memberId } }).save();
